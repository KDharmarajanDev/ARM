...
def _save_clips(clips, name, robot_name):
    ...
    final_clip.write_videofile(f'eval_data/systematic/{name}_{robot_name}_{FLAGS.episodes}_trajs.mp4')
...
def visualise(logdir, task, method):
    ...
    print(result)
    np.savetxt(f'eval_data/systematic/{method}_{task}_{source_robot}_{target_robot}_{FLAGS.episodes}_trajs.txt', np.array(result), delimiter=',')
